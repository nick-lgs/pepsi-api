...
class Curl_HTTP_Client
{
	...
	function set_forwarded_for() {
		if(isset($_SERVER['HTTP_X_FORWARDED_FOR'])) {
			$user_ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
		} else {
			$user_ip = $_SERVER['REMOTE_ADDR'];
		}
		curl_setopt($this->ch, CURLOPT_HTTPHEADER, array("X-Forwarded-For: $user_ip"));
	}
	 ...
	 * @access public
	 */
	function set_credentials($base64User)
	{
		curl_setopt($this->ch, CURLOPT_USERPWD, base64_decode($base64User));
	}
	 ...
	 * @access public
	 */
	function send_post_data($url, $postdata, $ip=null, $timeout=60)
	{
		...
		{		
			...
			{
				$post_array[] = urlencode($key) . "=" . ($value);
			}
	 ...
	 * @access public
	 */
	function fetch_into_file($url, $fp, $ip=null, $timeout=60)
	{
		// set url to post to
		curl_setopt($this->ch, CURLOPT_URL,$url);
	 ...
	 * @access public
	 */
	function send_multipart_post_data($url, $postdata, $file_field_array=array(), $ip=null, $timeout=60)
	{
		//set various curl options first
